...
const cloudGet = async (key) => {
  try { const r = await fetch(`/api/storage?key=${encodeURIComponent(key)}`); const j = await r.json(); return j.data ?? null; }
  catch { return null; }
};
const cloudSet = async (key, value) => {
  try { await fetch("/api/storage", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({key,value}) }); }
  catch {}
};
...
function useTheme() {
  const [dark, setDark] = useState(() => { if(typeof window!=="undefined") return window.matchMedia("(prefers-color-scheme: dark)").matches; return false; });
  ...
  return dark ? {
    ...
    green:"#30D158", red:"#FF453A", orange:"#FF9F0A", blue:"#0A84FF", dark:true,
  } : {
    ...
    green:"#34C759", red:"#FF3B30", orange:"#FF9500", blue:"#007AFF", dark:false,
  };
}

function Toast({ msg, T }) {
  if (!msg) return null;
  return <div style={{ position:"fixed", bottom:90, left:"50%", transform:"translateX(-50%)", background:T.text, color:T.bg, padding:"10px 20px", borderRadius:22, fontSize:13, zIndex:1000, whiteSpace:"nowrap", fontFamily:font, pointerEvents:"none", boxShadow:"0 4px 20px rgba(0,0,0,.2)" }}>{msg}</div>;
}
  ...
  useEffect(() => {
    (async () => {
      const [r,w,c,b,wm,aa,inst,s] = await Promise.all([
        cloudGet("records"),cloudGet("wallets"),cloudGet("cats"),cloudGet("budgets"),
        cloudGet("wordmap"),cloudGet("aa"),cloudGet("installments"),cloudGet("settings"),
      ]);
      if(r) setRecords(r); if(w) setWallets(w); if(c) setCats(c); if(b) setBudgets(b);
      if(wm) setWordMap(wm); if(aa) setAaList(aa); if(inst) setInstallments(inst); if(s) setSettings(s);
      setLoaded(true);
    })();
  }, []);
...
function ChatTab({ T, cats, records, wallets, budgets, wordMap, setWordMap, aaList, setAaList, installments, setInstallments, settings, addRecord, delRecord, updWallet, showToast }) {
  ...
  const budgetWarn = (cat,amt) => {
    ...
    if(pct>=1)return settings.alertStyle==="fun"?`🚨【${cat}】预算爆了！超 ${fmt(spent-budgets[cat])}`:`提示：本月【${cat}】超支 ${fmt(spent-budgets[cat])}。`;
    if(pct>=0.8)return settings.alertStyle==="fun"?`📊【${cat}】只剩 ${Math.round((1-pct)*100)}% 了～`:`提示：本月【${cat}】已用 ${Math.round(pct*100)}%，剩 ${fmt(budgets[cat]-spent)}。`;
    return null;
  };
  ...
  const finishRecord = (p,category) => {
    ...
    if(p.isTransfer){
      if(p.fromWallet&&p.toWallet){
        updWallet(p.fromWallet,-p.amount); updWallet(p.toWallet,p.amount);
        const fw=wallets.find(w=>w.id===p.fromWallet)?.name||p.fromWallet;
        const tw=wallets.find(w=>w.id===p.toWallet)?.name||p.toWallet;
        push("bot",`🔄 已划转 ${fmt(p.amount)}\n${fw} → ${tw}`);
      }
      return;
    }
    ...
    if(p.isAA&&p.people?.length>0){
      const totalPeople=p.people.length+1;
      const myShare=parseFloat((p.amount/totalPeople).toFixed(2));
      const toCollect=parseFloat((p.amount-myShare).toFixed(2));
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const system=`你是个人财务解析引擎（马来西亚，货币RM）。将输入解析为JSON数组。
支持：普通支出、多笔（逗号分隔）、AA分摊、分期付款、收入、转账、AA收款。
字段：type(expense/income/transfer)、desc、amount、category(从[${cats.expense.join(",")}|${cats.income.join(",")}]选)、walletId(从[${walletNames}]匹配,默认cash)、fromWallet、toWallet、isAA、people(AA欠款人数组)、isInstallment、totalMonths、monthlyAmount、isTransfer、isAASettle、personName、tags(2-3个语义标签)、confidence(high/low)。
只返回JSON数组，不要任何解释。`;
      const raw=(await callAI(system+"\n\n用户输入："+text)).replace(/```json|```/g,"").trim();
  ...
  const handleQuery = async (text) => {
    ...
    try {
      ...
      const instSummary=installments.filter(i=>!i.completed).map(i=>`分期:${i.desc} 每月${i.monthlyAmount} 已还${i.paidMonths?.length||0}/${i.totalMonths}期`).join("\n");
      const answer=await callAI(`你是私人财务助理，货币RM，简洁中文回答（≤4句）。\n\n记录：\n${summary}\n\n分期：\n${instSummary}\n\n问：${text}`);
      setLoading(false);
      push("bot","📊 "+(answer||"暂无法回答").trim());
...
function WalletsTab({ T, wallets, setWallets, records, showToast }) {
  ...
  const ICONS=["🏦","💳","💵","🟡","🔵","🟢","🔴","🟤","🧡","💜","⚫","🟩"];
  const COLORS=["#FFCC00","#0066CC","#1E90FF","#00B14F","#EE4D2D","#8B5CF6","#F59E0B","#8B4513","#FFD700","#006400","#00008B","#4ECDC4"];
